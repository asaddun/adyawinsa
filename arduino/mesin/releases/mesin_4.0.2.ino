        ...
        if (wiFiConnected == false || websocketConnected == false){
          ...
          if (file && websocketConnected == true) {
            ...
            if (cycleTime <= maxCycleTime){
              ...
              unsigned long minuteDowntime = secondDowntime / 60; // Downtime in minute
              // action = "down";
...
/*
this sketch:
Add downtime counter from inject if greater that maxCycleTime = 300 (5 minutes).
Add downtime counter wifi disconnected in the loop.
Add downtime counter wifi try connect in the setup.
Add timeout for autoconnect wifimanager in the setup if several time not get configured
then restarting the ESP/back to the beginning the setup/try connect wifi again.
When disconnect from wifi, cycletime will stored in FS file, will send when reconnected to wifi
and receive Inject signal.
*/
